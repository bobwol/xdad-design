...
exports.hashAndSub = function(grunt, options) {
  ...
  hashresFormatter =  utils.hashresFileNameFormat(fileNameFormat);
    ...
    options.files.forEach(function(f) {
      f.src.forEach(function(src) {
        var md5       = utils.md5(src).slice(0, 8),
            ...
            hashresNamed = hashresFormatter({
              name: fileName.slice(0, lastIndex),
              ext : fileName.slice(lastIndex + 1, fileName.length) });

        // Mapping the original name with hashed one for later use.
        nameToHashedName[fileName] = {renamed:renamed, hashresNamed:hashresNamed};
      ...
      grunt.file.expand(f.dest).forEach(function(f) {
        ...
        for (var name in nameToHashedName) {
          ...
          var file_name_regexp = new RegExp(preg_quote(name), "g");
          if (destContents.search(file_name_regexp)==-1){
            file_name_regexp = new RegExp(preg_quote(nameToHashedName[name]['hashresNamed'].replace(/\$\{hash\}/g, "wwww")).replace(/wwww/g,'\\w*'),"g")
          }
          destContents = destContents.replace(file_name_regexp, nameToHashedName[name]['renamed']);
        }
