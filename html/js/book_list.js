...
function clt_List_content_get_progress_data(opt,callback){
	...
	gServer.getJson(contentUrl, function(data) {
	...
	var gCLTAPP = new CLTAPP({
		dependents:{
			"List_project":{
				...
				// 		'<h4 data-id="" data-name="type_categorized"><a href="javascript:void(0);">所有</a></h4>',
				// 		'{{/if}}',
				// 		'{{else}}',
				// 		'<h4 data-id="" data-name="type_categorized"><a href="javascript:void(0);">所有</a></h4>',
				// 		'{{/if}}',
				...
				// 		'	<li>',
				// 		'		<h4 data-id="${id}" data-name="type_categorized"><a href="javascript:void(0);">${title}</a></h4>',
				// 		'		<ol>',
			...
			*/
			'List_content':{
				options:{
					...
					// },
					get_data:function(opt,callback){
						clt_List_content_get_progress_data(opt,callback);
					},
					item_click:function(id){
						object_url = $("tr[data-id='"+id+"']").attr('data-url');
						global_categories_load(object_url,'div#JS_list_detail');
					},
					template:[
						'<thead>',
						'  <tr>',
				        ...
				        '  <tr class="${review_state}" data-id="${id}" data-url="${url}">',
						...
						'		{{each(n,name) operationalitems}}',
      					'			{{if type=="workflow"}}',
                        '        		<li><a href="${url}" data-id="${id}" id="${id}" class="workflow">${globaldict_lookup(name)}</a></li>',
                        '			{{else}}',
                        '        		<li><a href="${url}/${id}" data-id="${id}">${globaldict_lookup(name)}</a></li>',
                        '			{{/if}}',
						'		{{/each}}',
                        '    	</ul></div>',
						'	{{/if}}',
				        ...
				        '    <td>${created}</td>',
						'    <td>${creator}</td>',
						'    <td><span class="published">${globaldict_lookup(review_state)}</span></td>',
				        '  </tr>',
				        '{{/each}}',
				    	'</tbody>',
					].join("")
