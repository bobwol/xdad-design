...
var iPageDirection='landscape';//initial the page attributes ,always the page set to landscape ,only use for mobile system
var currentDiv;//return the current element for handle
var elementfocus=false;//judge for current select element lock when element change happen for reservation protection
var squarefixxed=false;
var ratiofixxed=false;
var errormessage={
	numbererror:'integral number required',
	positionerror:'Please input correct position value'
};
/* end of define global varibles */
...
function model_render(id,data){//this is a very useful function which render the data fatch by fatchall function which get all the model back at once 
	...
	_.each(data,function(model){
		...
		if(!iHandlemodel.icollection.get(model.id)) iHandlemodel.icollection.add(model);
		...
		{},
		{
			...
			success: function(model,response){
				...
				if(returned.Status=="Success"){
					model.id=returned.ID.toString();//set the new model id
...
function model_new(id,type,options){//create new model when a button click to spcific type button
	//the id point to the new create model's parent (also data server parent 
	//so use collection to create ,the model accroding to it's specfic parent collection must be define ahead
	//add options to new model ,this options are some control set such as 
	if(elementfocus==true){
		model_confirm(iCurrentModel.id);
	}
	if(!options) options={createfocus:true};
	var iModel=iElementlist.get(id);
	iModel.icollection.add({iType:type,iCommon:_.clone(commonpattern[type]),iDetail:_.clone(detailpattern[type]),iOptions:options});
}
...
/* for site 
* function list to handle for the whole site , like preview , place mask ,and etc.
*/
function site_setpreview(){
	$('.element').addClass('preview');
}
function site_resetpreview(){
	$('.element').removeClass('preview');
}
function site_setmodal(){//when you find it is difficult to design the element you want ,you can use modal to filter other elements
	$('body').append('<span class="mask modal ui-widget-overlay"></span>');
}
function site_resetmodal(){
	$('span.mask.modal').remove();
}
function site_hide_tag(){
	$('.element .iType').hide();
}
function site_show_tag(){
	$('.element .iType').show();
}
/* @end */
...
function div_find(id){//throught id to find the Div object 
	var querydiv;
	//$('div.element #divid').each(function(){
	//	if($(this).val()==id) {
	//		querydiv=$(this).parent();
	//	}
	//})
...
function div_settop(id,zindex){//bring the div to front 
	if(!zindex) zindex=1000;
	var iHandlediv=div_find(id);
	iHandlediv.css('z-index',zindex);
}
function div_setparenttop(id,zindex){//when we use modal sometimes we want to set the layout or overlay to top so that 
	//we can edit the layout elements apperance inside the layout directly instead of confirm operate to change to it's parent
	if(!zindex) zindex=1000;
	var iHandlediv=div_find(id);
	iHandlediv.parent('div.element').css('z-index',zindex);
}
function div_resettop(id){//set the div back to its original zindex
	var iHandlediv=div_find(id);
	iHandlediv.css('z-index','auto');
}
function div_resetparenttop(id){//just like above but this function take action to parent node
	var iHandlediv=div_find(id);
	iHandlediv.parent('div.element').css('z-index','auto');
}
function div_set_selected(id){//this function set the shade border to make the selected object marked
	var iHandlediv=div_find(id);
	iHandlediv.children('span#shade').addClass('selected');
}
function div_reset_selected(id){//the corresponding to above
	var iHandlediv=div_find(id);
	iHandlediv.children('span#shade').removeClass('selected');
}
...
function div_createnavfield(id){
	var iHandlediv=div_find(id);
	iHandlediv.append('<span class="iNavfield component"></span>');
}
function div_createstylefield(id){//sometimes wo want to set some background field or border fields to display the effect ,we use this function
	var iHandlediv=div_find(id);
	$('<span class="iStylefield component"></span>').insertAfter(iHandlediv.children('.iShade'));
}
function div_createimagefield(id){
	var iHandlediv=div_find(id);
	iHandlediv.append('<span id="imagefield" class="iImagefield component absolute"></span>');
}
function div_createelementshade(id){
	var iHandlediv=div_find(id);
	var iHandlemodel=iElementlist.get(id);
	if(iHandlemodel.iview) var border_type=iHandlemodel.iview.border_type;
	else border_type='solid';
	iHandlediv.append('<span id="shade" class="iShade '+border_type+' component"></span>');
}
...
// what button to set the control area need to be customed
	var iHandlediv=div_find(id);
	iHandlediv.append('<span id="control" class="iControl component"><button id="confirm" onclick="model_confirm(\''+id+'\')">confirm</button><button id="remove" onclick="model_removeconfirm(\''+id+'\')">remove</button></span>');
...
function div_createtextfield(id){
	var iHandlediv=div_find(id);
	$('<div class="iTextfield component absolute"></div>').insertAfter(iHandlediv.children('.iShade'));
}
...
function div_createaddDiv(id,type){ //the func for create div element 
	...
	parentdiv.append("<div id='"+id+"' data-id='"+id+"' class='element "+type+"'></div>");
	var iHandlediv=div_find(id);
	...
	if(global_config.element_outline==false) iHandlediv.addClass('nooutline');
	if(iHandlemodel.iview&&iHandlemodel.iview.element_position=='relative') iHandlediv.addClass('relative');
	div_createtypefield(id,type);
	div_createimagefield(id);
	div_createelementshade(id);
}
function div_creatediv_patch(id,position){
	if(position=='relative'){
		var iHandlediv=div_find(id);
		iHandlediv.addClass('relative');
		iHandlediv.children('div.iTextfield').addClass('relative');
	}
}
...
function div_reset_relative_div(id){//reset the element size by id
	var iHandlemodel=iElementlist.get(id);
	var common=iHandlemodel.get('iCommon');
	var commonattr=common[iPageDirection];
	var iHandlediv=div_find(id);
...
function div_resize(id){
	var iHandlemodel=iElementlist.get(id);
	var iParentdiv=div_find(iHandlemodel.get('iParentdiv'));
	if(iHandlemodel.get('iParentdiv')=='body') iParentdiv=$('body');
	var iHandlediv=div_find(id);
	iHandlediv.resizable({
		containment: iParentdiv ,
		zIndex: 2700 ,
		resize: function(event, ui) {
			var width = $(event.target).width();
			var height= $(event.target).height();
			iHandlemodel.trigger('resizing',{iWidth:width,iHeight:height});
			//div_resizehandle(id,parseInt(width),parseInt(height));
		},
...
function div_drag(id){//for div drag for specific id model
	var iHandlemodel=iElementlist.get(id);
	var iParentdiv=div_find(iHandlemodel.get('iParentdiv'));//acrroding to it's parent
	if(iHandlemodel.get('iParentdiv')=='body') iParentdiv='body';
	var iHandlediv=div_find(id);
	iHandlediv.draggable({
		containment: iParentdiv ,
		zIndex: 2700 ,
		create: function(event,ui){
		},
		drag: function(event, ui) {
			...
			iHandlemodel.trigger('dragging',{iStartx:parseInt(Pos.left),iStarty:parseInt(Pos.top)});
			//div_draghandle(id,parseInt(Pos.left),parseInt(Pos.top));
			div_hidebuttons($(this).children('.iType').attr('id'));
		},
		stop: function(event,ui){
			var Pos=$(this).position();
			iHandlemodel.trigger('dragstop',{iStartx:parseInt(Pos.left),iStarty:parseInt(Pos.top)});
			div_showbuttons($(this).children('.iType').attr('id'));
		}
	});
}

function div_keyscontrol(id){//for keys control,the keyscontrol function can only be used when a model is focused , can not be used for multipule system
	$(document).bind('keydown',function(event,ui){
		var iHandleelement=iElementlist.get(id);
		var common=iHandleelement.get('iCommon');
		if(event.shift){
			squarefixxed=true;
		}
		...
		if(code==37){//left
			...
			check=coordinationcheck(id,common[iPageDirection].iStartx-1,common[iPageDirection].iStarty,common[iPageDirection].iWidth,common[iPageDirection].iHeight);
			if(check=='ok'){
				common[iPageDirection].iStartx=common[iPageDirection].iStartx-1;
				//common[iPageDirection].iWidth=common[iPageDirection].iWidth+1;
				iHandleelement.trigger('change:div');
				iHandleelement.trigger('change:form');
			}
		}
		if(code==38){//up
			event.preventDefault();
			check=coordinationcheck(id,common[iPageDirection].iStartx,common[iPageDirection].iStarty,common[iPageDirection].iWidth,common[iPageDirection].iHeight+1);
			if(check=='ok'){
				common[iPageDirection].iStarty=common[iPageDirection].iStarty-1;
				//iWidth,common[iPageDirection].iHeight=common[iPageDirection].iHeight+1;
				iHandleelement.trigger('change:div');
				iHandleelement.trigger('change:form');
			}
		}
		...
		if(code==40){//down
			event.preventDefault();
			check=coordinationcheck(id,common[iPageDirection].iStartx,common[iPageDirection].iStarty+1,common[iPageDirection].iWidth,common[iPageDirection].iHeight);
			if(check=='ok'){
				common[iPageDirection].iStarty=common[iPageDirection].iStarty+1;
				iHandleelement.trigger('change:div');
				iHandleelement.trigger('change:form');
...
function div_releasekeyscontrol(id){
	$(document).unbind('keydown');
}
function div_setelementvisibility(id){
	var iHandleModel=iElementlist.get(id);
	var iHandlediv=div_find(id);
	if(iHandleModel.get('iVisibility')==true){
		iHandleModel.trigger('divhide');
	}
	else {
		iHandleModel.trigger('divshow');
	}
}
...
function dom_item_focus(id){//focus the relative dom, make the accroding dom to be highlighted 
	var iHandledom=dom_find(id);
	iHandledom.addClass('selected');
}
function dom_item_unfocus(id){//focus the relative dom, make the accroding dom to be highlighted 
	var iHandledom=dom_find(id);
	iHandledom.removeClass('selected');
}
...
function info_window_create(){
	$('div.info_window').dialog({
			...
			create:function(event, ui){
				...
				$('.info_window_dialog').css('position','fixed');
			},
...
function info_loadattributes(id){//this function load the type attributes to info area 
	var ihandlemodel=iElementlist.get(id);
	$('div#info_attributes').html($('div#forminfo').children('#'+ihandlemodel.get('iType')).html());
}
...
function info_resetattributes(id){
	$('div#info_attributes').empty();
}
function info_resetdom(id){
	$('div#info_dom').children('.accordion').empty();
}
function info_resetresources(id){
	$('div#info_resources').empty();
}
...
function info_bindattributes_input(id){//bind all the input changes to connected the model handle functions 
		var iHandlemodel=iElementlist.get(id);
...
function buttons_window_create(){
	...
	$('div.buttons_window').dialog({
			...
			create:function(event, ui){
				...
				$('.buttons_window_dialog').css('position','fixed');
				buttons_actions();
			},
			open:function(event,ui){
...
function text_window_create(){
	$('body').append('<div class=\"text_window\"><textarea id="text_window_area" class="text_window_area" style="width:500px;"></textarea></div>');
...
function myCustomOnInit() {
	if(iCurrentModel){
		...
		$('table.mceLayout').children('tbody').children('tr.last').css('position','fixed');
		$('table.mceLayout').children('tbody').children('tr.last').css('background','#FFFFFF');
		$('table.mceLayout').children('tbody').children('tr.last').css('top','0px');
		$('table.mceLayout').children('tbody').children('tr.last').css('left','0px');
		$('table.mceLayout').children('tbody').children('tr.mceLast').css('position','fixed');
		$('table.mceLayout').children('tbody').children('tr.mceLast').css('background','#FFFFFF');
		$('table.mceLayout').children('tbody').children('tr.mceLast').css('top','0px');
		$('table.mceLayout').children('tbody').children('tr.mceLast').css('left','0px');
		var element_position='absolute';
		if(iCurrentModel.iview) element_position=iCurrentModel.iview.element_position;
		if(element_position=='absolute') {
			$('table.mceLayout iframe').addClass('fixfullheight');
			$('table.mceLayout iframe').contents().find('html').addClass('overflowy');
		}
		$('table.mceLayout iframe').css('background','transparent');
		iCurrentModel.iview.resettinymcefield();
		iCurrentModel.iview.aftertinymceload();
	}
}
function text_tinymce_init(){
	//$('body').append('<div class=\"text_window\" class="hidden"><textarea id="text_window_area" class="text_window_area" style="width:500px;"></textarea></div>');
	tinyMCE.init(tinymce_config);//you can set your coustom config for tinymce
}
...
function init_global_actions(){//this function define the global click actions which will be used globalized
	...
	$('a.resources_upload').live('click',function(){//this live to the resources button handle
		iCurrentModel.trigger('resources_beforeopen',$(this).attr('id'));
	})
	$('a.text_upload').live('click',function(){
		iCurrentModel.trigger('text_beforeopen',$(this).attr('id'));
	})
	$('a.dom_visibility').live('click',function(){//this the global function to set div show and hide
		var id=$(this).attr('id');
		div_setelementvisibility(id);
	})
	$('a.dom_move').live('click',function(){
		var id=$(this).attr('id');
		var iHandlemodel=iElementlist.get(id);
...
function init_global_div_selectable(container){//this function initialset the element selectable actions, 
	//when a element's span area is selected , the element can be focused or handled 
...
function init_global_div_element_selectable(container){//this function initialset the element selectable actions, 
	//when a element selected , the element can be focused or handled 
	$(container).delegate('div.element','dblclick',function(event,ui){
		if(iCurrentHoverModel){
			iCurrentHoverModel.trigger('focus');
		}
	})
}
function init_global_div_hover(container){
	$(container).delegate('div.element','mouseover mouseout',function(event,ui){
		var id=$(this).attr('id');
		iHandlemodel=iElementlist.get(id);
		if (event.type == 'mouseover') {
			iHandlemodel=iElementlist.get(id);
			iHandlemodel.trigger('mouseover');
		} else {
			iHandlemodel=iElementlist.get(id);
			iHandlemodel.trigger('mouseout');
		}
	})
}
function init_global_dom_selectable(){
	$('div#info_dom').delegate('li.dom_item','mouseover mouseout',function(event,ui){
		id=$(this).attr('id');
		iHandlemodel=iElementlist.get(id);
		if (event.type == 'mouseover') {
			iHandlemodel=iElementlist.get(id);
			iHandlemodel.trigger('highlight');
			$(this).addClass('highlight');
		} else {
			iHandlemodel=iElementlist.get(id);
			iHandlemodel.trigger('cancelhighlight');
