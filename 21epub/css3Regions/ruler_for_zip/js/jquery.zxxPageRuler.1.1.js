...
(function($){
	$.pageRuler = function(params){
		...
		var f = {
			box: function(){
				w = $(params.container).width()+40, h = $(params.container).height()+40;
				//整个box的宽高
				x.height(h).width(w);
				//弹出层的定位
				bg.css({
		...
		//参考线的水平拖移
		$(".zxxRefLine_v").bind("mousedown", function(){
			oDrag = $(this);	
			dragFlag = true;
			f.dashv();
		});
		//参考线的垂直拖移
		$(".zxxRefLine_h").bind("mousedown", function(){
			oDrag = $(this);	
			dragFlag = true;
			f.dashh();
		});
