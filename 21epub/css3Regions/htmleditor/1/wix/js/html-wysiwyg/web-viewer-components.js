...
W.Components.newComponent({name:"wysiwyg.viewer.components.AdminLoginButton",skinParts:{label:{type:"htmlElement"},link:{type:"htmlElement"}},propertiesSchemaName:"ButtonProperties",traits:["mobile.core.components.traits.LinkableComponent"],Class:{Extends:"wysiwyg.viewer.components.SiteButton",initialize:function(c,b,a){this.parent(c,b,a)
}}});
W.Components.newComponent({name:"wysiwyg.viewer.components.ClipArt",skinParts:{link:{type:"htmlElement"},img:{type:"mobile.core.components.Image",dataRefField:"*"}},propertiesSchemaName:"WPhotoProperties",traits:["mobile.core.components.traits.LinkableComponent"],Class:{EDITOR_META_DATA:{general:{settings:true,design:false},custom:[{label:"CLIP_ART_REPLACE_IMAGE",command:"WEditorCommands.OpenImageDialog",commandParameter:{galleryTypeID:"clipart"}},{label:"LINK_LINK_TO",command:"WEditorCommands.OpenLinkDialogCommand",commandParameter:{position:"center"},commandParameterDataRef:"SELF"}]},Extends:"wysiwyg.viewer.components.WPhoto",initialize:function(c,a,b){this.parent(c,a,b);
...
W.Components.newComponent({name:"wysiwyg.viewer.components.EbayItemsBySeller",skinParts:{iFrameHolder:{type:"htmlElement"}},propertiesSchemaName:"EbayItemsBySellerProperties",Class:{EDITOR_META_DATA:{general:{settings:false,design:true}},Extends:"mobile.core.components.base.BaseComponent",Binds:["_applyThemeColors"],_renderTriggers:[Constants.DisplayEvents.MOVED_IN_DOM,Constants.DisplayEvents.ADDED_TO_DOM,Constants.DisplayEvents.DISPLAYED,Constants.DisplayEvents.DISPLAY_CHANGED],_states:["hasContent","noContent"],initialize:function(c,a,b){b=b||{};
this.parent(c,a,b);
...
if(a||this._iframe.src!==this._getUrl()){this.setIFrameSrc(this._getUrl())
}this._iframe.width=this.getWidth()+"px";
this._iframe.height=this.getHeight()+"px"
},setIFrameSrc:function(a){this._iframe.src=this._getUrl()
...
W.Components.newComponent({name:"wysiwyg.viewer.components.LinkBar",skinParts:{imageItem:{type:"wysiwyg.viewer.components.LinkBarItem",repeater:true,container:"itemsContainer",dataRefField:"items",argObject:{unit:"px"}},itemsContainer:{type:"htmlElement"}},imports:[],propertiesSchemaName:"LinkBarProperties",Class:{EDITOR_META_DATA:{general:{settings:true,design:false},custom:[{label:"SOCIAL_BAR_MNG_ICONS",command:"WEditorCommands.OpenListEditDialog",commandParameter:{galleryConfigID:"icons"},commandParameterDataRef:"SELF"}]},Extends:"mobile.core.components.BaseRepeater",Binds:["_setComponentSize"],initialize:function(c,a,b){this.parent(c,a,b);
this._resizableSides=[];
this.setMaxH(9999);
this.setMaxW(9999)
},_onDataChange:function(a){this.parent()
...
return{left:(a*(this._itemWidth+this._gap)),top:(c*(this._itemHeight+this._gap))}
...
W.Components.newComponent({name:"wysiwyg.viewer.components.VerticalLine",skinParts:{},Class:{EDITOR_META_DATA:{general:{settings:false,design:true}},Extends:"mobile.core.components.base.BaseComponent",_renderTriggers:[Constants.DisplayEvents.SKIN_CHANGE],Binds:["render"],initialize:function(c,a,b){this.parent(c,a,b);
this.injects().Theme.setProperty("WEB_THEME_DIRECTORY","viewer");
this._resizableSides=[W.BaseComponent.ResizeSides.TOP,W.BaseComponent.ResizeSides.BOTTOM]
...
W.Components.newComponent({name:"wysiwyg.viewer.components.Video",skinParts:{videoFrame:{type:"htmlElement"},preview:{type:"htmlElement"}},propertiesSchemaName:"VideoProperties",Class:{EDITOR_META_DATA:{general:{settings:true,design:true}},Extends:"mobile.core.components.base.BaseComponent",Binds:["_stopVideo","_getYouTubeParams","_getVimeoParams","_onResize","_onResizeEnd"],_renderTriggers:[Constants.DisplayEvents.DISPLAY_CHANGED],_defaultPlayerParams:{wmode:"transparent"},_options:{videoType:"",videoId:"",resize:[]},_getServices:function(){return{YOUTUBE:{url:"http://www.youtube.com/embed/",preview:this._getYouTubePreview.bind(this),getParams:this._getYouTubeParams,hMinSize:200,wMinSize:200},VIMEO:{url:"http://player.vimeo.com/video/",preview:this._getViemoPreviewUrl.bind(this),getParams:this._getVimeoParams,hMinSize:100,wMinSize:100}}
...
W.Components.newComponent({name:"wysiwyg.viewer.components.WPhoto",skinParts:{link:{type:"htmlElement"},img:{type:"mobile.core.components.Image",dataRefField:"*",hookMethod:"_initializeWithPassedArgs"}},propertiesSchemaName:"WPhotoProperties",traits:["mobile.core.components.traits.LinkableComponent"],Class:{EDITOR_META_DATA:{general:{settings:true,design:true},custom:[{label:"PHOTO_REPLACE_IMAGE",command:"WEditorCommands.OpenImageDialog",commandParameter:{galleryTypeID:"photos"}},{label:"LINK_LINK_TO",command:"WEditorCommands.OpenLinkDialogCommand",commandParameter:{position:"center"},commandParameterDataRef:"SELF"}]},Extends:"mobile.core.components.base.BaseComponent",_renderTriggers:[Constants.DisplayEvents.ADDED_TO_DOM,Constants.DisplayEvents.DISPLAYED,Constants.DisplayEvents.DISPLAY_CHANGED,Constants.DisplayEvents.SKIN_CHANGE],Binds:["render"],initialize:function(c,a,b){this.parent(c,a,b);
...
if((e=="PREVIEW"||e=="VIEW")&&this._skinParts){var k=$(this._skinParts.richTextContainer).getElements("a");
for(var d=0;
d<k.length;
d++){var h=k[d];
...
},_bindDdlToData:function(a){for(var b=0;
b<this._optionsData.length;
b++){var c=this._optionsData[b];
a.options[b]=new Option(c.get("text"),c.get("value"))
}if(this.getComponentProperty("hasPrompt")){a.options.add(new Option(this.getComponentProperty("promptText"),-1,true,true),0);
this._optionsData.splice(0,0,null)
}this._setSelected(a)
...
W.Components.newComponent({name:"wysiwyg.viewer.components.inputs.TextInput",skinParts:{input:{type:"wysiwyg.viewer.components.inputs.TextInput"},errMsg:{type:"htmlElement"}},Class:{Extends:"mobile.core.components.base.BaseComponent",setErrorMessage:function(a){this._skinParts.errMsg.set("html",a)
},getAcceptableDataTypes:function(){return["Text"]
}}});
...
W.Components.newComponent({name:"wysiwyg.viewer.components.sm.SMContainer",imports:[],skinParts:{blockingLayer:{type:"htmlElement"},button:{type:"htmlElement"},content:{type:"htmlElement"},errMsg:{type:"htmlElement"},headerLabel:{type:"htmlElement"},headerSubLabel:{type:"htmlElement"},cancel:{type:"htmlElement",command:"WViewerCommands.SiteMembers.Close"}},Class:{Extends:"mobile.core.components.base.BaseComponent",Binds:["_handleSubmit","_onFormError","_onFormSuccess","_getCurrentForm"],_states:{forms:["register","login"]},initialize:function(c,a,b){this.parent(c,a,b);
this._formType=b&&b.formType
},_onAllSkinPartsReady:function(b){var a=b.button;
a.addEvent("click",this._handleSubmit);
b.blockingLayer.addEvent("click",function(){if(event.target==b.blockingLayer){this.injects().Commands.executeCommand("WViewerCommands.SiteMembers.Close",null,null)
}}.bind(this));
this._setActiveForm(this._formType)
},_setActiveForm:function(a){if(this._currentForm){this._removeCurrentFormEvents();
this._currentForm.getViewNode().removeFromDOM();
delete this._currentForm;
this._skinParts.errMsg.set("html","");
this._skinParts.headerSubLabel.set("html","")
}W.Components.createComponent("wysiwyg.viewer.components.sm.SM"+a.capitalize(),"wysiwyg.viewer.skins.sm.SM"+a.capitalize()+"Skin",null,{},null,function(h){this._currentForm=h;
var j=this._skinParts.headerLabel;
j.set("html",this._currentForm.getDisplayName());
var d=this._skinParts.button;
d.set("value",this._currentForm.getButtonLabel());
var g=this._skinParts.headerSubLabel;
var b=this._currentForm.getSubHeaderElement();
if(b){var e=b.el;
var f=b.formType;
e.getElements("a").addEvent("click",function(){this._setActiveForm(f)
}.bind(this));
g.appendChild(e)
...
},_onFormError:function(a){this._skinParts.errMsg.set("html",a)
},_onFormSuccess:function(a){this._skinParts.errMsg.set("html","Logged In!");
location.reload()
},_handleSubmit:function(){this._skinParts.errMsg.set("html","");
this._getCurrentForm().performSubmit()
},_editModeChanged:function(b,a){this.injects().Commands.executeCommand("WViewerCommands.SiteMembers.Close",null,null)
},_getCurrentForm:function(){return this._currentForm
}}});
W.Components.newComponent({name:"wysiwyg.viewer.components.sm.SMForm",imports:[],Class:{Extends:"mobile.core.components.base.BaseComponent",Binds:["_onFormError","_onFormSuccess"],Static:{PASS_MIN_LEN:4,PASS_MAX_LEN:15,ERR_MAP:{GENERAL_ERR:"Server error. try again later.",VAL_ERR_PASSWORD_BLANK:"Password cannot be blank",VAL_ERR_PASSWORD_RETYPE:"Passwords are not the same",VAL_ERR_PASSWORD_LENGTH:"Password length must be between {0} and {1}",VAL_ERR_EMAIL_BLANK:"Email cannot be blank",VAL_ERR_EMAIL_INVALID:"Email is invalid","-19999":"Unknown user","-19995":"Email is already taken","-19972":"Invalid token","-19988":"Validation Error","-19984":"Invalid Session","-19976":"Invalid Credentials"}},performSubmit:function(){LOG.reportError(wixErrors.MISSING_METHOD,this.className,"performSubmit")
...
},_onFormError:function(b){var a=this._getErrorMessage(b);
this.fireEvent("error",a)
},_onFormSuccess:function(){this.fireEvent("success")
},_getErrorMessage:function(e){var f=e.errorCode;
var d=this.ERR_MAP.GENERAL_ERR;
...
},_bindPasswordField:function(a){a.argObject={passwordField:true};
...
W.Components.newComponent({name:"wysiwyg.viewer.components.sm.SMLogin",imports:[],skinParts:{email:{type:"wysiwyg.viewer.components.inputs.TextInput",hookMethod:"_bindTextDataItem"},password:{type:"wysiwyg.viewer.components.inputs.TextInput",hookMethod:"_bindPasswordField"}},Class:{Extends:"wysiwyg.viewer.components.sm.SMForm",_onAllSkinPartsReady:function(){this._emailField=this._skinParts.email;
this._passwordField=this._skinParts.password
},performSubmit:function(){var b=this._emailField._data.get("text");
var a=this._passwordField._data.get("text");
if(this._validateFields(b,a)){this.injects().SiteMembers.login(b,a,this._onFormSuccess,this._onFormError)
...
this._onFormError({errorCode:"VAL_ERR_EMAIL_BLANK"});
return false
}else{if(!this.injects().Utils.isValidEmail(b)){this._emailField.setValidationState(false);
this._onFormError({errorCode:"VAL_ERR_EMAIL_INVALID"});
return false
}else{if(!a){this._passwordField.setValidationState(false);
this._onFormError({errorCode:"VAL_ERR_PASSWORD_BLANK"});
return false
}else{if(a.length<this.PASS_MIN_LEN||a.length>this.PASS_MAX_LEN){this._passwordField.setValidationState(false);
this._onFormError({errorCode:"VAL_ERR_PASSWORD_LENGTH",errorParams:[this.PASS_MIN_LEN,this.PASS_MAX_LEN]});
return false
}}}}return true
},getDisplayName:function(){return"Sign in:"
},getButtonLabel:function(){return"GO!"
},getSubHeaderElement:function(){var a=new Element("DIV");
a.set("html","Or, request <a>membership</a>");
return{el:a,formType:"register"}
}}});
W.Components.newComponent({name:"wysiwyg.viewer.components.sm.SMRegister",imports:[],skinParts:{email:{type:"wysiwyg.viewer.components.inputs.TextInput",hookMethod:"_bindTextDataItem"},password:{type:"wysiwyg.viewer.components.inputs.TextInput",hookMethod:"_bindPasswordField"},re_password:{type:"wysiwyg.viewer.components.inputs.TextInput",hookMethod:"_bindPasswordField"}},Class:{Extends:"wysiwyg.viewer.components.sm.SMForm",_onAllSkinPartsReady:function(){this._emailField=this._skinParts.email;
this._passwordField=this._skinParts.password;
...
this._onFormError({errorCode:"VAL_ERR_PASSWORD_RETYPE"});
return false
}}}}}return true
},getDisplayName:function(){return"Sign up:"
},getButtonLabel:function(){return"Sign up!"
},getSubHeaderElement:function(){var a=new Element("DIV");
a.set("html","I already have a user <a>Login</a>");
return{el:a,formType:"login"}
}}});
...
W.Classes.newTrait({name:"wysiwyg.viewer.components.traits.ValidationSettings",trait:{Static:{VALID_STATE_CHANGED_EVENT:"validStateChanged"},_isValid:true,_isSelfValidating:false,_validationFunction:function(a){return true
},initialize:function(c,a,b){this.parent(c,a,b);
if(b.validationFunction){this.setValidationFunction(b.validationFunction)
}if(b.isSelfValidating){this._isSelfValidating=b.isSelfValidating
