...
},readItem:function(b,d,c,a){var e=function(f){Object.each(f.referencedItems,function(j,i){if(!this._cache.isItemMarkedAsDeleted(i)){this._cache.putIfNotDirty(i,j)
}}.bind(this));
var g=this._cache.key(b,f.item._iid);
var h=this._cache.isItemMarkedAsDeleted(g)?null:this._cache.putIfNotDirty(this._cache.key(b,f.item._iid),f.item);
c(h)
}.bind(this);
this._api.readItem(b,d,e,a)
},query:function(f,d,b,e,a,g,c){var h=function(j){Object.each(j.referencedItems,function(n,m){if(!this._cache.isItemMarkedAsDeleted(m)){this._cache.putIfNotDirty(m,n)
}}.bind(this));
var i=[];
var k=[];
j.items.forEach(function(n){var m=this._cache.key(f,n._iid);
if(!this._cache.isItemMarkedAsDeleted(m)){var o=this._cache.get(m);
if(!(o&&o.isDirty())){i.push(this._cache.put(m,n));
k.push({_type:"wix:Ref<"+n._type+">",collectionId:f,itemId:n._iid})
}}}.bind(this));
var l=false;
...
},putIfNotDirty:function(b,c){var a=this._items[b];
if(!a){a=this._dataItemFactory.createDataItem(c);
this._items[b]=a
}else{if(!a.isDirty()){a.setValue(c)
}}return a
...
W.Classes.newClass({name:"wixapps.core.logics.CategoryLogic",imports:[],Class:{Extends:"wixapps.core.logics.SingleItemLogic",Binds:[],generatePanel:function(a,b,c){this._environment.getDataService().query("Categories",null,null,null,null,function(d){var f=[];
for(var e=0;
e<d.length;
...
},run:function(c,b,a){},getDataEditingModel:function(){},_getZoomParams:function(d,f,c){f=f||"title";
c=c||"_iid";
...
W.Classes.newClass({name:"wixapps.core.logics.SingleItemLogic",imports:["wixapps.core.logics.DescriptorDataEditingModel"],Class:{Extends:"wixapps.core.logics.LogicBase",Binds:[],_environment:null,initialize:function(a){this._environment=a
},run:function(h,g,e){var d=this._environment;
...
W.Classes.newClass({name:"wixapps.core.views.ProxyFactory",imports:["wixapps.core.views.ViewEnvironment","wixapps.core.views.ViewContext"],Class:{Binds:[],_viewRepository:null,_dataItemFactory:null,_typesManager:null,_managers:null,_defaultByTypeAndMode:{},_byName:{},_eventDispatcher:null,initialize:function(b,c,d,a){this._viewRepository=d;
...
a.forEach(function(g){var k=g.mode||c;
