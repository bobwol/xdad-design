...
var wixEvents = {
	...
	EDITOR_ENTER_PAGE: {
		desc: "Mobile editor page change",
		type: l.type.funnel,
		category: l.category.editor
	},
	EMAIL_SEND: {
		desc: "Site url sent to mail",
		type: l.type.userAction,
		category: l.category.editor
	},
	BACK_TO_EDIT_FROM_CONGRATS: {
		desc: "Back to edit page from congrats page",
		type: l.type.userAction,
		category: l.category.editor
	},
	ADD_PAGE: {
	...
	ADD_COMPONENT: {
		desc: "Component added",
		type: l.type.userAction,
		category: l.category.editor
	},
	REMOVE_COMPONENT: {
		desc: "Component removed",
		type: l.type.userAction,
		category: l.category.editor
	},
	REORDER_COMPONENT: {
		desc: "Component reorder",
		type: l.type.userAction,
		category: l.category.editor
	},
	SITE_READY: {
		desc: "Mobile site ready",
		type: l.type.timing,
	...
	PREVIEW_DOM_LOADED: {
		...
		timerId: "load",
		thresholdTime: 3000,
		thresholdError: "PREVIEW_DOM_DELAY"
	},
	EDITOR_READY: {
		desc: "Mobile editor ready",
		type: l.type.timing,
		category: l.category.editor,
		timerId: "load",
		thresholdTime: 8000,
	...
	SEO_CHECKED_IN_SEO_PANEL: {
		desc: '"allow search engines to find my site" was selected in SEO panel',
		type: l.type.userAction,
		category: l.category.editor,
		biEventId: 226,
		biAdapter: "hed"
	...
	APPS_FLOW_ADDON_CLICKED: {
		desc: "FLOW: Apps - AddOns clicked",
		type: l.type.funnel,
		category: l.category.editor,
		biEventId: 231,
		timerId: "main",
...
var wixErrors = {
	...
	SITE_DOM_DELAY: {
		errorCode: 14001,
		desc: "Site DOM not ready in time",
		type: l.type.timing,
		category: l.category.viewer,
		issue: l.issue.timing,
		severity: l.severity.warning
	...
	PREVIEW_READY_DELAY: {
		errorCode: 14004,
		desc: "Preview not ready in time",
		type: l.type.timing,
		category: l.category.viewer,
		issue: l.issue.timing,
		severity: l.severity.warning
	...
	MENU_CORRUPTION_UNKNOWN_PAGE: {
		errorCode: 22007,
		desc: "MAIN_MENU has unknown page",
		type: l.type.error,
		category: l.category.viewer,
		issue: l.issue.modal,
		severity: l.severity.recoverable
...
var WixGoogleAnalytics = function(p, n, o, r, s, q, m) {
		this._wixAccounts = p;
...
WixGoogleAnalytics.prototype.sendEvent = function(m, i) {
	var g = wixLogLegend.getKey("type", m.type);
	var j = m.desc;
	var h = i.label;
	var k = i.value || i.time;
	this._sendAnalyticEvent(g, j, h, k)
};
WixGoogleAnalytics.prototype.sendError = function(o, q, j, k) {
	var p = wixLogLegend.getKey("type", o.type);
	var m = o.desc;
	var i = q + "." + j + " : " + k.label;
	var n = k.value || k.time;
	this._sendAnalyticEvent(p, m, i, n)
};
WixGoogleAnalytics.prototype.sendPageEvent = function(c) {
	window._gaq = window._gaq || [];
	var d = function(a, j, h) {
			for (var i = 0;
			i < a.length; ++i) {
				if (a[i].length > 0) {
					var b = (i === 0) ? "" : "t" + i + ".";
					...
					if (h) {
						window._gaq.push([b + "_setCustomVar", 1, "version", this._version, 1]);
	...
	if (this._sendPageTrackToUser) {
		d(this._userAccounts, c, false)
	} else {
		if (this._sendPageTrackToWix) {
			d(this._wixAccounts, c, true)
		}
	}
};
WixGoogleAnalytics.prototype._sendAnalyticEvent = function(i, h, o, k) {
	i = i || "";
	h = h || "";
	o = o || "";
	k = k || 0;
	var j = function(c, a, f, e, d, b) {
			window._gaq = window._gaq || [];
	...
	if (this._wixAccounts) {
		for (var n = 0;
		n < this._wixAccounts.length; ++n) {
			var m = (n === 0) ? "" : "t" + n + ".";
			j(m, this._wixAccounts[n], i, h, o, k)
		}
	}
};
var WixBILogger = function(m, s, o, u, p, q, n, r, k, t) {
		this._wixLogger = m;
		k = encodeURIComponent(k);
		t = t ? t : 3;
		this.logScript = null;
		this._floggerServerURL = (s.charAt(s.length - 1) !== "/") ? s + "/" : s;
		this._commonFieldsFiltersByAdapter = {
			hed: ["src", "evid", "ts", "esi"],
			trg: ["src", "did", "uid", "gsi", "cid", "ver", "lng", "evid", "ts", "esi", "cat", "errc", "iss", "sev", "errscp", "trgt", "dsc"]
		};
		this._common = {
			src: t,
			did: u,
...
WixBILogger.prototype.setSrc = function(b) {
	this._common.src = b
};
WixBILogger.prototype.setDocId = function(b) {
	this._common.did = b
};
WixBILogger.prototype.setMetaSiteId = function(b) {
	this._common.msid = b
};
WixBILogger.prototype.setEditorSessionId = function(b) {
	this._common.esi = b
};
WixBILogger.prototype.sendError = function(o, v, r, u, n) {
	if (!o) {
		return
	}
	...
	if (typeof u == "string") {
		...
		t = (W && W.Utils) ? W.Utils.getStackTrace() : ""
	}
	t = encodeURI(t);
	var w = {
		errc: s,
...
WixBILogger.prototype.sendEvent = function(e, g) {
	var h = this.generateEventString(e, g);
	var f = e.biAdapter ? e.biAdapter : "mee";
	this._createReport(f, h, this._common)
};
WixBILogger.prototype.generateEventString = function(d, f) {
	if (!d) {
		return ""
	}
	f = f || {};
...
WixBILogger.prototype._createReport = function(f, g, e) {
	var h = this._objToURLParamsStringWithFilter(e, this._commonFieldsFiltersByAdapter[f]);
	this._createHit(this._floggerServerURL + f + "?" + h + g)
};
WixBILogger.prototype._createHit = function(c) {
	if (debugMode && !(debugMode == "unit_test")) {
		var d = new Image(0, 0);
		d.src = c
	}
};
WixBILogger.prototype._combineArraysToString = function(f, h) {
	var i = "";
	for (var g = 0;
	g < f.length;
	g++) {
...
WixBILogger.prototype._combineObjectToString = function(f) {
	var e = [];
	for (var d in f) {
		e.push(d + "=" + f[d])
	}
...
WixBILogger.prototype._objToURLParamsStringWithFilter = function(j, g) {
	var h = "";
	for (var f in j) {
		if (!g || (g.indexOf(f) != -1)) {
			var i = (!j[f] && j[f] !== 0) ? "" : j[f];
...
var WixLogger = function(c) {
		this.reset();
		this._settings = c;
		this.setDebugMode(c.debugMode || false);
		this._initGoogleAnalytics(c);
		try {
			this._wixBI = new WixBILogger(this, c.floggerServerURL, c.version, c.siteId, c.userId, c.userType, c.session, c.computerId, c.creationSource, c.wixAppId)
		} catch (d) {
			this._wixBI = {
				setDocId: function() {},
				sendError: function() {},
...
WixLogger.prototype._initGoogleAnalytics = function(d) {
	var e = (d.enableGoogleAnalytics !== false);
	this._analytics = {
		sendError: function() {},
		sendEvent: function() {},
	...
	if (e) {
		try {
			this._analytics = new WixGoogleAnalytics(d.wixAnalytics, d.userAnalytics, d.version, d.userType, d.userLanguage, d.sendPageTrackToUser, d.sendPageTrackToWix)
		} catch (f) {}
	}
};
WixLogger.prototype.reset = function() {
	this._logList = [];
	this._timeData = {};
	this._initTime = new Date().getTime()
};
WixLogger.prototype.getSessionTime = function() {
	return new Date().getTime() - this._initTime
};
WixLogger.prototype.updateSetting = function(f, d) {
	var e = this._settings || {};
	e[f] = d;
	switch (f) {
	case "siteId":
		this._wixBI.setDocId(d);
		break;
	case "wixAppId":
		this._wixBI.setSrc(d);
		break;
	case "editorSessionId":
		this._wixBI.setEditorSessionId(d);
		break;
	case "metaSiteId":
		this._wixBI.setMetaSiteId(d);
		break
	}
};
WixLogger.prototype.getLog = function() {
	return this._logList.concat()
};
WixLogger.prototype.clearLog = function() {
	this._logList = []
};
WixLogger.prototype.setDebugMode = function(b) {
	this._debugMode = b
};
WixLogger.prototype.reportError = function(h, e, f, g) {
	if (h.ignore) {
		return
	}
	g = g || {};
	if (this._handleLogObj("reportError", h, g, {
		className: e,
		methodName: f
	})) {
		this._wixBI.sendError(h, e, f, g);
...
WixLogger.prototype.reportEvent = function(d, c) {
	c = c || {};
	if (this._handleLogObj("reportEvent", d, c)) {
		if (d.biEventId) {
			this._wixBI.sendEvent(d, c)
		}
		this._analytics.sendEvent(d, c);
		this._settings.onEvent && this._settings.onEvent(d, c)
	}
};
WixLogger.prototype.reportEditorLoadingEvent = function(e, f) {
	var d = window.viewMode;
	if (d != "site") {
		this.reportEvent(wixEvents.LOADING_STEPS, {
			c1: f + ":" + d + ": " + e
		})
	}
};
WixLogger.prototype.reportPageEvent = function(b) {
	if (W.Config.getDebugMode() !== "nodebug") {
		return
	}
	this._analytics.sendPageEvent(this._sanitizePageUrl(b, window.viewMode))
};
WixLogger.prototype._sanitizePageUrl = function(d, c) {
	if (c == "editor") {
		d = this._sanitizeUrlForWixBI(d)
	}
	return d
};
WixLogger.prototype._sanitizeUrlForWixBI = function(b) {
	return b
};
WixLogger.prototype.addErrors = function(b) {
	Object.forEach(b, this.addError.bind(this))
};
WixLogger.prototype.addError = function(c, d) {
	if (!(d in this._settings.errors) && this.isUniqueErrorCode(c.errorCode)) {
		this._settings.errors[d] = c
	} else {
		throw new Error("Invalid error object: " + d)
	}
};
WixLogger.prototype.isUniqueErrorCode = function(b) {
	return Object.every(this._settings.errors, function(a) {
		return b !== a.errorCode
	})
};
WixLogger.prototype._handleLogObj = function(j, g, i, h) {
	if (!g) {
		return false
	}
	g.callCount = g.callCount || 0;
	if (g.callLimit && g.callLimit <= g.callCount) {
		return false
	} else {
		this._checkTime(g, i);
		this._logLog(j, g, i, h);
		if ((g.thresholdTime || g.thresholdTime === 0) && i.time >= g.thresholdTime) {
			if (g.thresholdError) {
				var f = this._settings.errors[g.thresholdError];
				this.reportError(f)
			}
		}
		g.callCount++
...
WixLogger.prototype._logLog = function() {
	this._logList.push(arguments)
};
WixLogger.prototype._checkTime = function(d, c) {
	c = c || {};
	c.time = this._getTime(d.timerId)
};
WixLogger.prototype._getTime = function(e) {
	e = e || "initTime";
	var d = this._initTime;
	var f = new Date().getTime();
	if (e != "initTime") {
		d = this._timeData[e] || this._initTime;
		this._timeData[e] = f
	}
	return f - d
};
Object.merge(wixErrors, {
	INVALID_INPUT_BIND: {
		errorCode: 26001,
