;(function (window,$){
	...
	function DragResize(myName, config) {
		var props = {
			...
			// Name of the object.
			enabled: true,
			// Global toggle of drag/resize.
			handles: ['tl', 'tm', 'tr', 'lm', 'rm', 'bl', 'bm', 'br'],
			// Array of drag handles: top/mid/bot/right.
			isElement: null,
			// Function ref to test for an element.
			isHandle: null,
			// Function ref to test for move handle.
			element: null,
			...
			minWidth: 10,
			minHeight: 10,
			// Minimum pixel size of elements.
			minLeft: 0,
			maxLeft: 9999,
			...
			maxTop: 9999,
			zIndex: 1,
			// The highest Z-Index yet allocated.
			mouseX: 0,
			mouseY: 0,
			// Current mouse position, recorded live.
			lastMouseX: 0,
			lastMouseY: 0,
			// Last processed mouse positions.
			mOffX: 0,
			mOffY: 0,
			...
			// Element size.
			allowBlur: true,
			// Whether to allow automatic blur onclick.
			ondragfocus: null,
		...
		this["o_minWidth"] = config['minWidth'];
		this["o_minHeight"] = config['minHeight'];
	...
	DragResize.prototype.select = function(newElement) {
		with(this) {
			...
			if (newElement && (newElement != element) && enabled) {
				...
				elmX = getoffset(element).left;
				elmY = getoffset(element).top;
	...
	DragResize.prototype.mouseMove = function(e) {
		with(this) {
			...
			if(this.pushKnob == 'tm'){
				...
				var dX = diffX,
				dY = diffY;
				if (elmX + dX < _minLeft) mOffX = (dX - (diffX = _minLeft - elmX));
				else if (elmX + elmW + dX > _maxLeft) mOffX = (dX - (diffX = _maxLeft - elmX - elmW));
				// ...and up-down.
				if (elmY + dY < _minTop) mOffY = (dY - (diffY = _minTop - elmY));
				else if (elmY + elmH + dY > maxTop) mOffY = (dY - (diffY = maxTop - elmY - elmH));
				
				if(elmX + dX < _minLeft){
					elmX= _minLeft;
				}else if(elmX + elmW + dX > _maxLeft){
					elmX= _maxLeft-elmW;
				}else {
					elmX += diffX;
				}
			...
			}else  if (this.resizeHandleDrag && this.resizeHandleDrag(diffX, diffY)) {
				isResize = true;
			}
			else {
				// If the resize drag handler isn't set or returns fase (to indicate the drag was
			...
			// if($p.hasClass("dragdropresize")){
				var ppos = $p.offset();
				with(ref.style) {
					left = elmX-ppos.left + 'px';
					width = elmW + 'px';
					top = elmY-ppos.top + 'px';
					height = elmH + 'px';
				}
			}else{
				with(ref.style) {
