		...
		__init : function () {
			...
			this.get_container()
				...
				.delegate(editarea, "mousedown.jstree", $.proxy(function (e) { 
						...
						if($(target).attr('data-original-title')!='上下拖拽')return;
					...
					case ($.isFunction(s.data)):
						s.data.call(this, obj, $.proxy(function (d) {
							if(d && d !== "" && d.toString && d.toString().replace(/^[\s\n]+$/,"") !== "") {
								...
								if(obj == -1 || !obj) { this.get_container().children("ul").empty().append(d.children()).find(editarea+","+editarea).filter(function () { return !this.firstChild || !this.firstChild.tagName || this.firstChild.tagName !== "SPAN"; }).prepend('<span class="btn-draged"></span>').end().filter(editarea).children("span:first-child").not(".jstree-icon").addClass("jstree-icon"); }
								else { obj.children(editarea+".jstree-loading").removeClass("jstree-loading"); obj.append(d).children("ul").find(editarea+","+editarea).filter(function () { return !this.firstChild || !this.firstChild.tagName || this.firstChild.tagName !== "SPAN"; }).prepend('<span class="btn-draged"></span>').end().filter(editarea).children("span:first-child").not(".jstree-icon").addClass("jstree-icon"); obj.removeData("jstree-is-loading"); }
								this.clean_node(obj);
					...
					case (!s.data && !!s.ajax) || (!!s.data && !!s.ajax && obj && obj !== -1):
						...
						success_func = function (d, t, x) {
							...
							if(d) {
								d = $(d);
								if(!d.is("ul")) { d = $("<ul />").append(d); }
								if(obj == -1 || !obj) { this.get_container().children("ul").empty().append(d.children()).find("li,"+editarea).filter(function () { return !this.firstChild || !this.firstChild.tagName || this.firstChild.tagName !== "SPAN"; }).prepend('<span class="btn-draged"></span>').end().filter(editarea).children("span:first-child").not(".jstree-icon").addClass("jstree-icon"); }
								else { obj.children(editarea+".jstree-loading").removeClass("jstree-loading"); obj.append(d).children("ul").find("li,"+editarea).filter(function () { return !this.firstChild || !this.firstChild.tagName || this.firstChild.tagName !== "SPAN"; }).prepend('<span class="btn-draged"></span>').end().filter(editarea).children("span:first-child").not(".jstree-icon").addClass("jstree-icon"); obj.removeData("jstree-is-loading"); }
								this.clean_node(obj);
								if(s_call) { s_call.call(this); }
							}
							else {
