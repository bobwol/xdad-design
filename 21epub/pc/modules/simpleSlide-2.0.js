...
epub.def(function (){
	...
	$.simpleSlide.prototype = {
		...
		_render:function(){
			...
			self.$indicatorSelecter = $(self.options.indicatorSelecter,this.$el[0]);
			...
			if (self.$indicatorSelecter.length>0) self.$indicatorSelecter.parent().css({'z-index':1000});
		...
		_showPics:function (index){
			...
			if (self.$indicatorSelecter.length>0) {
				$(self.$indicatorSelecter).attr('class','');
				$(self.$indicatorSelecter[self.index]).attr('class','active');
			}
	...
	$.simpleSlide.defaults = {
		...
		prevSelecter:".prev",
		nextSelecter:".next",
		indicatorSelecter:".indicators li",
		show:function (){}
	}
