...
//http://www.pdfdownload.org/web2pdf/Default.aspx?left=0&right=0&top=0&bottom=0&page=0&cURL='+document.location.href
epub.def(function (){
	...
	PageEdit.prototype = {
	    ...
	    init:function(){
			...
			var css = [
				    ...
				    '}',
				    '.'+this.regionClass+'{',
				    '	-ms-flow-from: contentIdentifier;',
			...
			this.$template.append($.tmpl(this.opt.templates.page,{}));
			this.$template.find(this.opt.regionSelecter).addClass(this.regionClass);

	        this.$previewSelecter = $('<div class="preview"></div>').hide();
	        this.$editSelecter.insertAfter(this.$previewSelecter)
