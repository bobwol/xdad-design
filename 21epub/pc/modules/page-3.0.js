	    ...
	    $.page.prototype = {
			...
			_goto:function(elem){
				...
				self._getunlitslist(function (data){
					...
					if ($('div.nano ul li').length>5){
						$('div.nano .pane').css('display','block');
					}else{
						$('div.nano .pane').css('display','none');
					}
			...
			_goto_unlitslist:function (target){
				...
				$.tmpl(this.options.templates.detail,{
					body:data_body,
					title:data_title
				}).appendTo(self.$detail);

				$('article.nanoscrollbar').wrapInner('<div class="nano"><div class="content"></div></div>');
				$('article.nanoscrollbar .nano').nanoScroller();
