...
<fieldset class="actionitem" style="margin:0">
	<legend><%=interaction.elementlist.get(overlay_id).toJSON().iDetail.title||interaction.elementlist.get(overlay_id).get("iType")%>(<%=global.dict.query(interaction.getActionTypeName(iType))%>)</legend>
...
<div style="text-align:center" class="item-save"><a class="btn btn-small btn-primary action-confirm" style="display:inline"><i class="icon-ok icon-white"></i></a></div>
</div>
  		  </div>
  </div>