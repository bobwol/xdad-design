...
interaction_view.model.Audio=interaction_view.model.Base.extend({
	defaults:{
	...
	setview:function(type){
		this.iview=new interaction_view.view.Audio({model:this});
	},
	setsyncmodel:function(){
	}
})
interaction_view.view.Audio = interaction_view.view.Base.extend({
	events: {
	...
	preload:function(){
		...
		this.model.played=false;
		this.model.time=0;
		if(detail.iFile&&detail.iFile[0]){
			var file=detail.iFile[0].file||detail.iFile[0].content;
			var preload=new interaction_view.model.Preload({file:file,overlay_id:this.model.id,elementtype:this.model.get('iType'),pageid:this.model.get('pageid')});
		}		
	},
