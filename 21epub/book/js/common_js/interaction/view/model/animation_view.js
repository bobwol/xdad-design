	...
	setPlay:function(pageid){
		...
		this.each(function(i){
			...
			if(JSON.iTiming.start==0) {
				...
				collection.timeline.call(collection.AddClickTween,[collection],this,collection.TotalDuration);
			}
			else if(JSON.iTiming.start==2) {
				if(currentindex!=0){
					collection.timeline.addLabel(i.id+'_start',collection.at(currentindex-1).id+'_start');
