...
tinymce.PluginManager.add('lists', function(editor) {
	...
	editor.on('init', function() {
		...
		function indent() {
			var state, rng = selection.getRng(true), bookmark = createBookmark(selection.getRng(true));
			var container = rng['commonAncestorContainer'];

			if ($(container).children().length==getSelectedListItems().length){
				...
				indentStyleName = 'marginLeft';

				value = (parseInt(container.style[indentStyleName] || 0, 10) + intentValue) + indentUnit;
				dom.setStyle(container, indentStyleName, value);
		...
		function outdent() {
			...
			if ($(container).children().length==getSelectedListItems().length){
				...
				value = Math.max(0, parseInt(container.style[indentStyleName] || 0, 10) - intentValue);
				dom.setStyle(container, indentStyleName, value ? value + indentUnit : '');
