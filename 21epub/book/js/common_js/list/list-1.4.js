...
var templates = {
	...
	pm_List_detail:{
		...
		content:[
			...
			'         <li><span class="alignleft">开始日期：</span><span class="alignright">${planStart}</span></li>',
			'         <li><span class="alignleft">结束日期：</span><span class="alignright">${planEnd}</span></li>',

	        '	{{if roles.length}}',
			'		{{each(n,name) roles}}',
		    '		{{each name}}',
		    '{{if status == $index}}',
		    '         <li class="active"><span class="alignleft">${$index}：</span><span class="alignright">${$value}</span></li>',
		    '{{else}}',
		    '         <li><span class="alignleft">${$index}：</span><span class="alignright">${$value}</span></li>',
		    '{{/if}}',
	...
	pt_List_content:[
		...
		'			{{if enabled}}',
		'			{{if children.length}}',
		'        	<li id="${id}" class="new"><a href="＃">${name}</a>',
		'			{{else}}',
		'        	<li id="${id}"><a href="＃">${name}</a>',
		'    		{{/if}}',
		'			{{if children.length}}',
		'       		<div class="new-bd">',
		'      				<ul>',
	...
	pt_List_detail:{
        ...
        '    <!-- sub-project start -->',
        '    <dl class="task-management">',
        '      <dt style="position:relative;">标题：${title}</dt>',
...
List_project.prototype={
	...
	bindevent:function(callback){
		...
		self.selecter.click(function (e){
			...
			if(isFind){
				...
				callback(id);
				if(self.project_click)self.project_click(id);
				return self;
			}
		})
		return this;
	...
	getCurrentProject:function(){
		var self = this;
		return self.selecter.find(".active h4");
	}
}	
	...
	bindevent:function(callback){
		...
		self.selecter.click(function (e){
			...
			if(isFindB){
				...
				callback(id);
				if(self.progress_click)self.progress_click(id);
	...
	setDependents:function(){
		...
		for(var n in dependents){
			...
			if(modulseManger[module]){
				this[module] = this.dependents[module] = new modulseManger[module](opts.options);  
			}else{
...
PMLAPP.prototype.business = function(){
		...
		function List_detailbusiness_project(data){
			list_detail
				.render_project(templates.pm_List_detail.project,data)
		...
		function list_projectbusiness(data){
			...
			;(function(){
				...
				if(href.indexOf("id=")>-1){
					...
					id = arr[1];
					var target =""; 
					list_project.selecter.find("h4").each(function(){
						var $this = $(this);
					...
					id = firstProject.attr("data-id");
					$(firstProject).parent().parent().parent().find(".active").removeClass("active");
					$(firstProject).parent().parent().addClass("active");
				}
		...
		list_project.update = function(id,projectdata,progressdata){
			...
			var url = list_project.url;
			list_data.set(url,projectdata);
			var urla = list_content.url+"?id="+id;

			list_data.set(urla,progressdata);
			
			list_project
				.render(templates.pm_List_project,projectdata)
				
			list_menu
				.render(templates.pm_List_menu,projectdata);	
			var target =""; 
			list_project.selecter.find("h4").each(function(){
				var $this = $(this);
				var idA = $this.attr("data-id");
			...
			var project_lis = projectdata.project_lis;
			var ret = byIdGetData(id,project_lis)
			List_contentbusiness(progressdata)
			List_detailbusiness_project(ret)
			if(list_project.project_click)list_project.project_click(id);
			
		}
		list_content.update = function(id,data){
			...
			var projectid = project.attr("data-id");
			var url = list_content.url+"?id="+projectid;
			var content_trs = list_data.set(url,data);

			List_contentbusiness(data);
			var target =""; 
			list_content.selecter.find("tr").each(function(){
				var $this = $(this);
				var idA = $this.attr("data-id");
				if(id==idA){
					target = $this[0];
			...
			var content_trs = data.content_trs;
			var ret = byIdGetData(id,content_trs)
			List_detailbusiness_content(ret);
