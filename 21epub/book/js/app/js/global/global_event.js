...
function init_global_workflow_submit_button_event(){
	...
	set_click_event('div.modal#workflow button.btn',function(opt){
		var ret = {}, opt;
		object_id = $('div#workflow #objectid').val();
		...
		global.modal.hide('div#workflow');
		$('body').jswait();
		global.json.post(object_url + "/operatestate.json",ret,function(data){
			$('body').jswait('close');
			// modal_hide('div#workflow');
			if(data.code==200){
				global.message('success',data.msg);
				if (typeof units != "undefined"){
					units.model.icollection.get(object_id).set(data.data.results[0]);					
				}
				if (typeof editions_list != "undefined"){
...
function init_global_workflow_atag_click_event(){
	...
	set_click_event('a.workflow',function(opt){
		...
		var label = $(this).text();
		var object_id = $('a.workflow').closest('.content-item').attr('id');
		...
		if (transition=='retract'){
			...
			if (window.confirm("将收回当前发布内容，继续请点击'确定'按钮")){		
			...
			$("div#workflow").html(global_workflow_template({"label":label,"transition":transition,"object_url":object_url,"object_id":object_id}));
