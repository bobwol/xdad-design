...
var CoreViewer = function(c) {
    ...
    var g = function() {
		...
		*/
        var S = (h.metadata != null && !T);//->true
		/*
			"pageStatuses": "",
		*/
        var R = (h.pageStatuses.indexOf("0") == -1 && S);//->true
		// c= viewerParams = {
			// "id": "DocViewer",
			// "page": 1,
			// "zoom": "auto"
		// };
		//向#DocViewer中添加.doc的div 并给#DocViewer添加.docviewer
        J = y('<div class="doc"></div>').appendTo(y("#" + c.id).addClass("docviewer")).get(0);
		// if (!R) {
            // var Q = new StatusUpdater(J, h);//与socket通信
    ...
    var L = function() {
        ...
        var R = true;//(h.pageStatuses.indexOf("0") == -1 && h.metadata && h.status != 4);//->true
		
        var Z = (R) ? "full": "init";//->full
        var X = (y.browser.msie && y.browser.version <= 8) ? ("ie" + y.browser.version).substr(0, 3) : "std";//->std
        //获取样式
		var W = o + "css/" + X + "-" + Z + ".css";
		...
		*/
        for (var V = 0; V < d.numpages; V++) {
			...
			' height:{height}em;">'+
			'<div id="Page{num}" '+
			'class="page" style="width:{width}em;'+
			' height:{height}em;"></div></div>';
        ...
        //N(c.page || 1);
		/*
		J = y('<div class="doc"></div>')
		aa = h.pageStatuses;//""
		d = h.metadata;
		o = './'//h.assetsLocation;
		*/
        // b = new LazyLoader(J, aa, d, o);
        // y(k).bind("zoom resize statusupdate",
        // function(af) {
            // y(b).triggerHandler(af)
        // });
        // y(b).bind("pagechange unavailable",
        // function(af) {
...
var LazyLoader = function(l, a, n, d) {
	/*
		l = J = y('<div class="doc"></div>')
		a = aa = h.pageStatuses;//""
		n = d = h.metadata;
		d = o = './'//h.assetsLocation;
                ...
                if (aj.type == "drawing") {
                    ...
                    if (aj.type == "textbox") {
                        ...
                        if (aj.selectionRange) {
                            ...
                            if (Z.length > 1000 && !aa) {
                                ConfirmDialog.show("Are you sure you want to highlight the selected text?", null, null, null,
                                function(al) {
                                    if (al) {
                                        b(aj, true)
                                    }
            ...
            keydown: function(i) {
                if (b.selected) {
                    ...
                    if ((i.which == 46 || i.which == 8) && !h.is("textarea") && !h.attr("contenteditable")) {
                        ...
                        ConfirmDialog.show(g, null, null, null,
                        function(e) {
                            if (e) {
                                b.selected.remove()
                            }
...
var MouseEventSimulator = function(m) {
    ...
    var l = function(x) {
        ...
        var s = Math.min.apply(this, $.map(u,
        function(y) {
            return y.x
        }));
        var w = Math.min.apply(this, $.map(u,
        function(y) {
            return y.y
        }));
        var r = Math.max.apply(this, $.map(u,
        function(y) {
            return y.x + y.width
        }));
        var t = Math.max.apply(this, $.map(u,
        function(y) {
            return y.y + y.height
        }));
        d[q][v] = {
...
var CommentSet = function(l, C, c) {
    ...
    var g = function() {
        ...
        var I = [["M", J.x, J.y], ["L", H.x, H.y], ["L", G.x, G.y], ["L", D.x, D.y]];
        t.attr("path", I)
    };
    var A = function() {
...
var DocViewer = function(d) {
    ...
    var f = function(n) {
        ...
        $(k).bind("zoom resize modechange settingschange",
        function(p) {
            $(b).triggerHandler(p)
        });
        $(b).bind({
...
//m.objects, m.session, m.demo)
/*
"objects": [{
		"name": "Demo User",
		"created": 1371946355.0,
		"unsaved": true,
		"modified": 1371946355.0,
		"class": "User",
		"id": 1,
		"uuid": "74d12e37-cdec-3c7d-b228-02c2b76a5faf"
	}],
*/
    var l = this;
    this.get = function(t) {
...
var TextSelectionUtilities = function() {
    ...
    var g = function(H) {
        ...
        for (var G = H.childNodes.length - 1; G >= 0; --G) {
...
var RenderedArea = function(i, v, e, g) {
    ...
    var c = function() {
        ...
        if (s.editable) {
            ...
            $(q.node).bind("contextmenu mousedown",
            function(R) {
                return false
            })
        }
    ...
    var d = function(F) {
        ...
        if (E == "remove") {
            ...
            ConfirmDialog.show(D, null, null, 220,
            function(G) {
                if (G) {
                    y()
                }
...
var RenderedDrawing = function(o, z, L, h) {
    ...
    var D = function() {
        ...
        if (z) {
            ...
            $(s.node).add(g.node).bind("contextmenu",
            function(T) {
            ...
            $(s.node).bind("contextmenu mousedown",
            function(T) {
                return false
            })
        }
    ...
    var J = function() {
        ...
        var R = [];
        for (var S = 0; S < i.annotation.lines.length; S++) {
...
var RenderedHighlight = function(f, s, d, h) {
    ...
    var b = function() {
        ...
        var A = Math.max.apply(this, $.map(f.rectangles,
        function(P) {
            return P.x + P.width
        }));
        var C = Math.min.apply(this, $.map(f.rectangles,
        function(P) {
            return P.y
        }));
        var J = {
    ...
    var c = function(A) {
        ...
        if (z == "remove") {
            if (o.hasComments) {
                ...
                ConfirmDialog.show(y, null, null, 220,
                function(B) {
                    if (B) {
                        u()
                    }
...
var RenderedPoint = function(g, r, d, e) {
    ...
    var c = function(C) {
        ...
        if (B == "remove") {
            ...
            ConfirmDialog.show(A, null, null, 220,
            function(D) {
                if (D) {
                    v()
                }
    ...
    var j = function() {
        ...
        var E = 6 * x;
        var D = 3 * x;
        var C = [["M", A, F], ["l", -B / 2, -D], ["l", 0, -E], ["l", B, 0], ["l", 0, E], ["z"]];
        q.attr({
            path: C
        });
        n.anchorLocation = {
