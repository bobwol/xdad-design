var CoreViewer = function(c) {
	...
	var g = function() {
		...
		J = y('.doc');//y('<div class="doc"></div>').appendTo(y("#" + c.id).addClass("docviewer")).get(0);
		if (!R) {
	...
	var A = function(J, I) {
		...
		function G(N, M) {
			if (N.nodeType == 3) {
				...
				for (var L = 0, K = N.childNodes.length; L < K;
				++L) {
					G(N.childNodes[L], M)
				}
			}
	...
	var q = function(G) {
		...
		function H(K) {
			if (K.nodeType == 3) {
				...
				for (var J = 0; J < K.childNodes.length;
				++J) {
					H(K.childNodes[J])
				}
			}
		...
		function J(P) {
			...
			if (P == H) {
				...
				if (P.nodeType == 3) {
					...
					for (var O = 0, N = P.childNodes.length; !G && O < N;
					++O) {
						J(P.childNodes[O])
					}
				}
